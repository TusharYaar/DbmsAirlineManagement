...
router.post("/searchflight", function (req, res) {
  var sql =
    " select duration, price, flight.flight_number,departure_date,departure_time,arrival_date,arrival_time, " +
...
router.get("/searchflight/:flight", middleware.sessionChecker, function (req, res) {
  ...
  connection.query(sql, [flight], function (err, result, fields) {
    if (err) {
      ...
      var sql2 = "select seat_number FROM flight JOIN bookedflight ON flight.flight_number = bookedflight.flight_number " + "WHERE flight.flight_number = ?";
...
router.get("/showbookedflights", middleware.sessionChecker, function (req, res) {
  sql =
    "select price, duration, ticket_number, userid, bookedflight.flight_number, username, seat_number, departure_date,departure_time,arrival_date, " +
    "arrival_time, ap_des.airport_name as des_name, ap_des.airport_state as des_state, ap_des.airport_city as des_city, ap_des.airport_short AS des_short, " +
    "ap_dep.airport_name as dep_name, ap_dep.airport_state as dep_state, ap_dep.airport_city as dep_city, ap_dep.airport_short as dep_short " +
    ...
    "where userid = ? ORDER BY departure_date DESC";
  connection.query(sql, [req.session.userid], function (err, result, fields) {
    if (err) {
      ...
      // res.send(result);
      res.render("./user/showtic", { result: result });
    }
  });
});
